...
`);Pe.title=e.data.text.title;const P=Oe();P.title=e.data.webcam;const C=g("miniMASK");C.title="Donn\xE9es de ports de masques chirurgicaux";const j=g("miniROAD");j.title="Donn\xE9es de circulation routi\xE8re";const A=g("miniRETINA");A.title="Donn\xE9es m\xE9dicales de r\xE9tinographie";const I=g("miniTRASH");I.title="Donn\xE9es de d\xE9chets \xE0 trier";const bt=g("miniFRUIT");bt.title="Donn\xE9es de pommes et bananes";const f=Fe("localStorage"),u=pe("training-set-mlp-vs-knn",f),d=pe("test-set-mlp-vs-knn",f),x=me(u);x.title=e.data.browser.train;const q=me(d);q.title=e.data.browser.test;const gt=o(v(e.embedding.video.url,e.embedding.video.title,e.embedding.video.width,e.embedding.video.height));gt.title=e.embedding.video.title;const _e=o(`
<style>
.depliable img {
    display: none;
}
...
`);_e.title=e.embedding.text.title;const D=Ke();D.title=e.embedding.title;const K=o("En attente de la s\xE9lection d'une image...<br><br><br><br><br><br><br><br><br><br>");K.title=e.embedding.feature_title;const vt=x.$selected.filter(t=>t.length===1).map(([t])=>u.get(t)).awaitPromises(),ft=q.$selected.filter(t=>t.length===1).map(([t])=>d.get(t)).awaitPromises(),wt=vt.merge(ft);wt.subscribe(t=>{var i=t.x[0].map(function(n){return n=n.toFixed(2),n});K.$value.set('<p style="font-size:10px;">'.concat(i.join(", "),"</p>"))});async function xt(t){const s=new Image;return new Promise(i=>{s.addEventListener("load",async()=>{const n=ue(()=>{const m=Ye(ce(s),M(255));return[k.resizeBilinear(m,[z,z]),k.resizeBilinear(m,[80,80])]}),a=new ImageData(await fe(n[0]),z,z),r=document.createElement("canvas");await fe(n[1],r);const p=r.toDataURL("image/jpeg");n[0].dispose(),n[1].dispose(),i([a,p])}),s.src=t})}async function y(t,s,i,n,a,r){s=="train"?await u.clear():await d.clear();const p=new Array(r.length);for(let h=0;h<r.length;h++)p[h]=Array.from(Array(i),(l,b)=>[`https://www.lisn.upsaclay.fr/~tsanchez/marcelle_datasets/${t}/${s}/${a[h]}_${s}_${b+1}.${n}`,`${r[h]}`]);let m=Promise.resolve();p.forEach(h=>{h.forEach(([l,b])=>{m=m.then(async()=>{const[_,U]=await xt(l),N={type:"image",image:_,y:b,thumbnail:U,x:await D.process(_),raw_features:await et({width:3,height:3,max:1,bw:!0},_)};s=="train"?await u.create(N):await d.create(N)})})}),await m}C.$click.subscribe(async()=>{await y("miniMASK","train",10,"jpg",["masque-bien-mis","masque-mal-mis","pas-de-masque"],["Masque bien mis","Masque mal mis","Pas de masque"]),await y("miniMASK","test",10,"jpg",["masque-bien-mis","masque-mal-mis","pas-de-masque"],["Masque bien mis","Masque mal mis","Pas de masque"])});A.$click.subscribe(async()=>{await y("miniRETINA","train",10,"png",["sain","rd","dmla"],["Sain","R\xE9tinopathie Diab\xE9tique","DMLA"]),await y("miniRETINA","test",10,"png",["sain","rd","dmla"],["Sain","R\xE9tinopathie Diab\xE9tique","DMLA"])});I.$click.subscribe(async()=>{await y("miniTRASH","train",10,"jpg",["emballage","verre-blanc","verre-colore"],["Emballage","Verre blanc","Verre color\xE9"]),await y("miniTRASH","test",10,"jpg",["emballage","verre-blanc","verre-colore"],["Emballage","Verre blanc","Verre color\xE9"])});j.$click.subscribe(async()=>{await y("miniROAD","train",10,"jpg",["feu-orange","feu-rouge","feu-vert"],["Feu orange","Feu rouge","Feu vert"]),await y("miniROAD","test",10,"jpg",["feu-orange","feu-rouge","feu-vert"],["Feu orange","Feu rouge","Feu vert"])});const qt=o(v(e.training.video.url,e.training.video.title,e.training.video.width,e.training.video.height));qt.title=e.training.video.title;const R=g("Entra\xEEner le kNN");R.title="";const $=g("Entra\xEEner le MLP");$.title="";const ke=o(`
...
`);Se.title=e.evaluation.text.title;const Y=S({name:"MLP_train",dataset:u,dataStore:f}),Z=S({name:"KNN_train",dataset:u,dataStore:f}),X=S({name:"MLP_test",dataset:d,dataStore:f}),ee=S({name:"KNN_test",dataset:d,dataStore:f}),B=g(e.evaluation.knn.button.content);B.title=e.evaluation.knn.button.title;const T=g(e.evaluation.mlp.button.content);T.title=e.evaluation.mlp.button.title;const te=E(Y);te.title=e.evaluation.mlp.matrix.train;const se=E(Z);se.title=e.evaluation.knn.matrix.train;const ie=E(X);ie.title=e.evaluation.mlp.matrix.test;const ne=E(ee);ne.title=e.evaluation.knn.matrix.test;B.$click.subscribe(async()=>{w.ready||ge(new Error("Le classifieur kNN n'a pas \xE9t\xE9 entra\xEEn\xE9 sur les donn\xE9es d'entra\xEEnement")),await Z.clear(),await ee.clear(),await Z.predict(w,u),await ee.predict(w,d)});T.$click.subscribe(async()=>{w.ready||ge(new Error("Le classifieur MLP n'a pas \xE9t\xE9 entra\xEEn\xE9 sur les donn\xE9es d'entra\xEEnement")),await Y.clear(),await X.clear(),await Y.predict(L,u),await X.predict(L,d)});const Lt=o(v(e.deployment.video.url,e.deployment.video.title,e.deployment.video.width,e.deployment.video.height));Lt.title=e.deployment.video.title;const Ee=o(`
...
`);Ce.title=e.credits.text.title;const Rt=o(v(e.research.video.url,e.research.video.title,e.research.video.width,e.research.video.height));Rt.title=e.research.video.title;const je=o(`

Nous vous serions tr\xE8s reconnaissant de r\xE9pondre \xE0 ce <a href="https://vpn-02-dc1.inria.fr/+CSCO+0h75676763663A2F2F666261716E7472662E766165766E2E7365++/index.php/935454?lang=fr" target="_blank">questionnaire anonyme<a>.
<br>
<br>
...
<br><br>
<b>Exercice:</b> R\xE9entra\xEEner les classieurs d'images sur d'autres donn\xE9es.`);Ae.title="";const c=We({title:e.misc.dashboards.title,author:e.misc.author.name});c.page(e.introduction.name).sidebar().use(xe,"Conseils",qe);c.page(e.history.name).sidebar().use("Consignes",ye,"Outils",Le);c.page(e.data.name).sidebar(C,j,A,I).use("Consignes",Pe,"Outils",[V,x,q]);c.page(e.embedding.name).sidebar(D).use("Consignes",_e,"Outils",K,[x,q]);c.page(e.training.name).sidebar().use("Consignes",ke,"k plus proches voisins (kNN)",Ne,[J,R,G],"Perceptron multicouche (MLP)",Me,[H,$,Q],W);c.page(e.evaluation.name).sidebar().use("Consignes",Se,"Outils",[B,T],[se,te],[ne,ie]);c.page(e.deployment.name).sidebar(P).use("Consignes",Ee,"Outils",[x,q],"Pr\xE9dictions",[V,re,ae]);c.page("Tout en un").sidebar(C,j,A,I).use("Consignes",Ae,"Entra\xEEnement",[J,R,G],[H,$,Q],W,"\xC9valuation",[B,T],[se,te],[ne,ie],"D\xE9ploiement",[x,q],[V,re,ae]);c.page(e.research.name).sidebar().use(je);c.page(e.credits.name).use(Ce);c.settings.dataStores(f).datasets(u,d).models(w,L);c.show();
