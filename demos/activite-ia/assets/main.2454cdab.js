...
`);ke.title=e.training.mlp.text.title;const w=Fe({k:1,dataStore:f}).sync("mlp-vs-knn-knn"),K=pe(w);K.title=e.training.knn.params;const F=me(w);F.title=e.training.knn.progress;const y=Je({layers:[64],epochs:20,dataStore:f}).sync("mlp-vs-knn-mlp"),J=pe(y);J.title=e.training.mlp.params;const G=me(y);G.title=e.training.mlp.progress;const Q=Ge(y);Q.title=e.training.mlp.plot;I.$click.subscribe(()=>{w.train(u,"raw_features")});D.$click.subscribe(()=>{y.train(u)});const yt=o(v(e.evaluation.video.url,e.evaluation.video.title,e.evaluation.video.width,e.evaluation.video.height));yt.title=e.evaluation.video.title;const Me=o(`
...
`);Me.title=e.evaluation.text.title;const W=S({name:"MLP_train",dataset:u,dataStore:f}),H=S({name:"KNN_train",dataset:u,dataStore:f}),Y=S({name:"MLP_test",dataset:d,dataStore:f}),Z=S({name:"KNN_test",dataset:d,dataStore:f}),R=g(e.evaluation.knn.button.content);R.title=e.evaluation.knn.button.title;const $=g(e.evaluation.mlp.button.content);$.title=e.evaluation.mlp.button.title;const X=z(W);X.title=e.evaluation.mlp.matrix.train;const ee=z(H);ee.title=e.evaluation.knn.matrix.train;const te=z(Y);te.title=e.evaluation.mlp.matrix.test;const se=z(Z);se.title=e.evaluation.knn.matrix.test;R.$click.subscribe(async()=>{w.ready||he(new Error("Le classifieur kNN n'a pas \xE9t\xE9 entra\xEEn\xE9 sur les donn\xE9es d'entra\xEEnement")),await H.clear(),await Z.clear(),await H.predict(w,u),await Z.predict(w,d)});$.$click.subscribe(async()=>{w.ready||he(new Error("Le classifieur MLP n'a pas \xE9t\xE9 entra\xEEn\xE9 sur les donn\xE9es d'entra\xEEnement")),await W.clear(),await Y.clear(),await W.predict(y,u),await Y.predict(y,d)});const Lt=o(v(e.deployment.video.url,e.deployment.video.title,e.deployment.video.width,e.deployment.video.height));Lt.title=e.deployment.video.title;const Se=o(`
...
<br><br>
<b>Exercice:</b> R\xE9entra\xEEner les classieurs d'images sur d'autres donn\xE9es.`);je.title="";const c=We({title:e.misc.dashboards.title,author:e.misc.author.name});c.page(e.introduction.name).sidebar().use(fe,"Conseils",we);c.page(e.history.name).sidebar().use("Consignes",xe,"Outils",qe);c.page(e.data.name).sidebar(C,j).use("Consignes",ye,"Outils",[B,x,q]);c.page(e.embedding.name).sidebar(A).use("Consignes",Pe,"Outils",O,[x,q]);c.page(e.training.name).sidebar().use("Consignes",_e,"k plus proches voisins (kNN)",Ne,[K,I,F],"Perceptron multicouche (MLP)",ke,[J,D,G],Q);c.page(e.evaluation.name).sidebar().use("Consignes",Me,"Outils",[R,$],[ee,X],[se,te]);c.page(e.deployment.name).sidebar(L).use("Consignes",Se,"Outils",[x,q],"Pr\xE9dictions",[B,ne,ie]);c.page("Tout en un").sidebar(j,C,L).use("Consignes",je,"Entra\xEEnement",[K,I,F],[J,D,G],Q,"\xC9valuation",[R,$],[ee,X],[se,te],"D\xE9ploiement",[x,q],[B,ne,ie]);c.page(e.research.name).sidebar().use(Ce);c.page(e.credits.name).use(Ee);c.settings.dataStores(f).datasets(u,d).models(w,y);c.show();
